...
window.start = function(s) { window.update = true; setTimeout(updateImage, 500); }
window.start();
});";

		public SimpleServer(int port) : base(port) {}
		public Func<byte[]> ImageProviderFunc;

		public override void handleGETRequest(HttpProcessor p)
		{
			if (p.http_url == "/image.jpg") {
				byte[] data = ImageProviderFunc();
